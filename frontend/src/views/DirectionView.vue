<template>
<div>
    <Announcement></Announcement>
    <Navbar></Navbar>
    <div class="row mx-0 px-5 mb-4 mt-4">
        <div class="col-12 mb-5 px-4">
            <div class="row">
                <div class="col-12 text-center fs-title fw-normal custom-title-color mb-3">
                    Москва
                </div>
                <div class="col-12 fs-6 text-center fw-light mb-5">
                    <div class="row justify-content-center">
                        <div class="col-8">
                            ВЫ не можете уложиться в бюджет? Или просто не хотите продумывать всё до мелочей.
                            Данный сервис сделает это за вас, используя математические алгоритмы.
                        </div>
                    </div>
                </div>
                <div class="col-12">
                    <div class="row justify-content-center">
                        <img class="col-5" :src="require('@/assets/image 7.png')" alt="Overlay">
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="row mx-0 px-5 mb-4" data-aos="fade-up">
        <div class="col-12 px-0">
            <div class="row m-0">
                <div class="col-12 text-center fs-5 fw-light custom-purple-text-color mb-3">Что там интересного?</div>
                <div class="col-12 text-center fs-2 fw-light custom-title-color mb-3">
                    Факты & Нейросеть
                </div>
                <div class="col-12 fs-6 text-center mb-5">
                    <div class="row justify-content-center">
                        <div class="col-8 fw-light">
                            Здесь вы найдете уникальные и интересные факты о нашем городе,
                            которые были сгенерированы с помощью передовой нейросетевой технологии.
                            Наша нейросеть анализирует огромные объемы данных,
                            чтобы предложить вам самые захватывающие и удивительные сведения, которые вы, возможно,
                            еще не знали..
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="row mx-0 px-5 mb-4" data-aos="fade-up">
        <div class="col-12 mx-0 px-0">
            <div class="row px-0 mx-0">
                <div class="col-6 p-3 ps-0">
                    <div class="row mx-0 border custom-border-color p-5 ps-0">
                        <div class="text-center fs-4 mb-2">Количество жителей</div>
                        <div class="d-flex justify-content-center mb-3">
                            <div class="pb-2 border-bottom custom-border-color" style="width: 60px;"></div>
                        </div>
                        <div class="text-center fs-6 fw-light">Здесь вы найдете уникальные и интересные факты о нашем городе,
                            которые были сгенерированы с помощью передовой не</div>
                    </div>
                </div>
                <div class="col-6 p-3 pe-0">
                    <div class="row mx-0 border custom-border-color p-5">
                        <div class="text-center fs-4 mb-2">Количество жителей</div>
                        <div class="d-flex justify-content-center mb-3">
                            <div class="pb-2 border-bottom custom-border-color" style="width: 60px;"></div>
                        </div>
                        <div class="text-center fs-6 fw-light">Здесь вы найдете уникальные и интересные факты о нашем городе,
                            которые были сгенерированы с помощью передовой не</div>
                    </div>
                </div>
                <div class="col-6 p-3 ps-0">
                    <div class="row mx-0 border custom-border-color p-5">
                        <div class="text-center fs-4 mb-2">Количество жителей</div>
                        <div class="d-flex justify-content-center mb-3">
                            <div class="pb-2 border-bottom custom-border-color" style="width: 60px;"></div>
                        </div>
                        <div class="text-center fs-6 fw-light">Здесь вы найдете уникальные и интересные факты о нашем городе,
                            которые были сгенерированы с помощью передовой не</div>
                    </div>
                </div>
                <div class="col-6 p-3 pe-0">
                    <div class="row mx-0 border custom-border-color p-5">
                        <div class="text-center fs-4 mb-2">Количество жителей</div>
                        <div class="d-flex justify-content-center mb-3">
                            <div class="pb-2 border-bottom custom-border-color" style="width: 60px;"></div>
                        </div>
                        <div class="text-center fs-6 fw-light">Здесь вы найдете уникальные и интересные факты о нашем городе,
                            которые были сгенерированы с помощью передовой не</div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="row mx-0 px-5 mb-2" data-aos="fade-up">
        <div class="col-12 px-0">
            <div class="row m-0 py-5 border custom-border-color">
                <div class="col-12 text-center fs-5 fw-light custom-purple-text-color mb-3">Куда здесь можно сходить?</div>
                <div class="col-12 text-center fs-2 fw-light custom-title-color mb-3">
                    Интересные места города Москва
                </div>
                <div class="col-12 fs-6 text-center">
                    <div class="row justify-content-center">
                        <div class="col-8 fw-light">
                            Откройте для себя самые захватывающие и примечательные места нашего города! 
                            Наш раздел "Интересные места города" предлагает подборку мест, которые стоит посетить.
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="row mx-0 px-5 my-5" data-aos="fade-up">
        <div class="col-12 px-0">
            <div class="row m-0">
                <div class="mx-0 px-0">
                    <DirectionElementList></DirectionElementList>
                    <div class="col-12 d-flex justify-content-center">
                        <ButtonOne class="px-4 py-2 fs-5 rounded-pill">Показать еще</ButtonOne>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <Footer></Footer>
</div>
</template>

<script>
import DirectionElementList from "@/components/DirectionElementList"
export default {
    components: {
        DirectionElementList
    },
    data() {
        return {
            element: { id: 1, name: "Element 1" },
        };
    },
}
</script>

<style scoped>

</style>
